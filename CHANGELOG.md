...
- Scan/apply worker ownership now sits behind lifecycle/controller facades, while
  library, media-apply, and cleanup panel boundaries have focused contract tests and
  a checked-in complexity budget.
...
- Manifest v2 plugin capability/resource contracts with capability-diff approval, contract-bound fingerprints, explicit v1 migration approval, spawn-isolated hooks, timeout/output limits, and high-risk in-process hook blocking.
...
- Qt-free `unifile watch` recursively settles new arrivals for 500 ms before
  classification, supports review-only or collision-safe apply mode, and flushes
  pending work on `SIGINT`/`SIGTERM`.
...
- File inspector Related Files panel now ranks shared tags, photographer, date
  range, and filename-pattern matches, with symmetric durable manual links.
...
- Tag Library tags now round-trip through adjacent XMP sidecars: standard `dc:subject` keywords are imported on reopen, UniFile-managed tags are updated without removing third-party keywords, and source files remain untouched.
...
- Semantic embeddings: add optional local ONNX Runtime backends with CUDA-to-CPU fallback, batched mean pooling, Auto/ONNX/Ollama selection, and persisted model/provider settings while preserving the existing Ollama path.
- Semantic duplicate detection: optionally batch-encode local CLIP/SigLIP-compatible ONNX image models, cluster cosine matches at a configurable threshold, and review them in a dedicated Duplicate Finder tab without replacing exact or perceptual-hash results.
- Workflow scripting: add a trusted, AST-validated `unifile.script` API with a child-process timeout watchdog, serializable tag/file-operation commands, and an embedded Plugin Manager editor with validation and live debugger output.
- YAML plugin manifests: discover validated `plugin.yaml` packages with manifest-bound trust fingerprints, generate scaffolds with `unifile plugin create`, and browse a bounded HTTPS community index from Settings → Plugins.
- Headless deployment: add a Qt-free Flask API with API-key auth, reviewable scan plans, tag/search/report routes, cron scan/tag jobs with optional change digests, an admin status page, and a Docker Compose + Ollama template.
- TagStudio interoperability: add read-only SQLite import, additive TagStudio-compatible export, field/tag hierarchy preservation, cached thumbnail transfer, CLI commands, and Tag Library panel actions without modifying source files.
- Book Library: add EPUB/PDF/MOBI/AZW3 scanning, local metadata extraction, optional cached OpenLibrary/Google Books enrichment, deterministic genre/language/series/reading tags, and non-destructive Calibre `metadata.opf` export.
- Video Project Awareness: add read-only `.aep`, `.prproj`, `.drp`, and `.fcpbundle` media-reference discovery, shared/orphan/missing Project Audit reporting, CLI output, and explicit project metadata/tag application to resolved library assets.
- Mobile Companion: add an on-demand token-protected read-only LAN PWA with tag-library browsing, search, relative paths, in-memory image previews, installable manifest, and service-worker shell caching.
- Collaborative LAN Tagging: add library-scoped hashed user tokens, admin/editor/viewer permissions, administrator-only confidential-tag visibility, per-field write conflicts, shared rule/tag management, audit events, and CLI/server workflows.
- File Health Monitor: add persistent streaming SHA-256 baselines, expected-change acknowledgement, changed/missing/unstable-file diffs, JSON/CSV/text log export, headless `/verify` and scheduled verify jobs, CLI verification, and a dashboard-integrated integrity diff dialog.
- PC File Organizer timeline: add a Created/Modified date histogram with accessible From/To scrub controls, combined text/face/date filtering, and safe visibility for undated files.
- Keyboard Shortcuts dialog: review and rebind every application action from Settings, persist custom bindings, disable entries by clearing them, and reject duplicate or Windows-reserved combinations before saving.
...
- Customizable sidebar: drag section headers to reorder Organize, Tools, Library, Smart Views, and Profile, collapse any section, and reset the saved layout from Settings.
