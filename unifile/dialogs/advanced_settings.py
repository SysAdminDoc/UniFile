...
class AIProviderSettingsDialog(QDialog):
    ...
    def _build_ui(self):
        ...
        lbl = QLabel("Configure AI providers for classification. Providers are tried in priority order.")
...
class WhisperSettingsDialog(QDialog):
    ...
    def _build_ui(self):
        ...
        lbl = QLabel("Whisper transcribes audio/video files for content-based classification.")
        lbl.setWordWrap(True)
        lbl.setStyleSheet(f"color: {_t['muted']}; font-size: 11px;")
        layout.addWidget(lbl)
        ...
        status = QLabel(f"Status: {'Available' if avail else 'Not installed (pip install openai-whisper)'}")
...
class SemanticSearchSettingsDialog(QDialog):
    ...
    def _build_ui(self):
        ...
        lbl = QLabel("Semantic search uses embedding vectors to find files by meaning, not just keywords.")
        lbl.setWordWrap(True)
        lbl.setStyleSheet(f"color: {_t['muted']}; font-size: 11px;")
        layout.addWidget(lbl)
        ...
        status = QLabel(f"Embedding model: {'Available' if avail else 'Not available (requires Ollama + nomic-embed-text)'}")
        status.setStyleSheet(f"color: {_t['green'] if avail else '#ef4444'}; font-size: 11px;")
        layout.addWidget(status)

        count = idx.get_indexed_count()
        lbl_count = QLabel(f"Indexed files: {count:,}")
...
class EmbeddingSettingsDialog(QDialog):
    ...
    def _build_ui(self):
        ...
        lbl = QLabel(
            "Metadata embedding writes classification results back into file metadata "
            "(EXIF, XMP, ID3 tags, etc.) so other apps can read them.")
        lbl.setWordWrap(True)
        lbl.setStyleSheet(f"color: {_t['muted']}; font-size: 11px;")
        layout.addWidget(lbl)
        ...
        for fmt, avail in caps.items():
            lbl_fmt = QLabel(f"{'[OK]' if avail else '[--]'} {fmt}")
            lbl_fmt.setStyleSheet(f"color: {_t['green'] if avail else _t['muted']}; font-size: 11px;")
            g_lay.addWidget(lbl_fmt)
        layout.addWidget(grp)

        # Options
        self.chk_auto = QCheckBox("Auto-embed after Apply (write categories to file metadata)")
...
class LearningStatsDialog(QDialog):
    ...
    def _build_ui(self):
        ...
        lbl = QLabel("The adaptive learning engine improves classification by learning from your corrections.")
        lbl.setWordWrap(True)
        lbl.setStyleSheet(f"color: {_t['muted']}; font-size: 11px;")
        layout.addWidget(lbl)
        ...
        info.setStyleSheet(f"color: {_t['fg']}; font-size: 12px;")
